...
class CircusPerm(AsyncEventParser):
    def __init__(self, controller, name):
        ...
        self.url = 'https://www.circus-perm.ru/'
        self.headers = {
    ...
    async def get_info_about_event(self, id):
        ...
        if title in self.reformat_title:
            title = self.reformat_title.get(title, title)

        id = info_about.get("data").get("id")
        href = f'https://ticket-place.ru/widget/{id}/data|perm'
    ...
    async def body(self):
        ...
        for event in a_events:
            self.register_event(event[0], event[1], date=event[2], venue='Пермский цирк')